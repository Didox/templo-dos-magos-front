...
export default function ProductGrid({ selectedCategory, searchTerm }: ProductGridProps) {
    ...
    async function loadProdutos() {
      try {
        ...
        if (searchTerm) {
          ...
          response = await getProdutosPorCategoria(selectedCategory, currentPage)
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-6 text-center text-white">{sectionTitle}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {produtos.map((produto) => (
              <ProductCard
                ...
                color={produto.categoria ? getColorClass(produto.categoria.cor) : "bg-purple-400"}
              />
            ))}
          </div>
