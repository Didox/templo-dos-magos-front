...
export default function PedidoDetalhes({
  ...
  const converterParaNumero = (valor: string | undefined): number => {
    if (!valor) return 0;
    return (
      Number.parseFloat(valor.replace(/[^\d.,]/g, "").replace(",", ".")) || 0
    );
  };
            ...
            <div className="space-y-3">
              ...
              pedido.produtos.length > 0 ? (
                pedido.produtos.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center gap-4 bg-purple-700 p-3 rounded-lg"
                  >
                    ...
                    <div className="flex-grow">
                      <h4 className="font-medium">
                        {item.produto ? item.produto.nome : "Produto"}
                      </h4>
                      <p className="text-sm text-purple-300">
                        {formatarPreco(converterParaNumero(item.precoUnitario))}{" "}
                        x {item.quantidade || 1}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-yellow-400">
                        {formatarPreco(converterParaNumero(item.subtotal))}
                      </p>
                    </div>
                  </div>
                ))
